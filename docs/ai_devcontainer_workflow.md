...
- Supply-chain hardening: Dockerfile downloads now enforce SHA-256 for ninja, mold, gh, ccache/sccache, ripgrep, node, mrdocs, jq, awscli, and valgrind; vcpkg cloning no longer tracks `main` automatically (set `VCPKG_REF` to pin). GitHub SSH keys are preseeded in `known_hosts` so StrictHostKeyChecking stays enabled.
