...
mapfile -t containers < <("${DOCKER_CMD[@]}" ps -a --format '{{.Names}}' | grep -i "vsc-slotmap_${HOST_SUFFIX}_" || true)
...
mapfile -t feature_images < <("${DOCKER_CMD[@]}" images --format '{{.Repository}}:{{.Tag}}' | grep -i "vsc-slotmap_${HOST_SUFFIX}_" || true)
